...
export const seedDatabase = async () => {
  try {
    console.log('🌱 Starting Database Seeding for Jaipur Healthcare Network...');
    ...
    // 2. Seed Doctors across Hospitals & Specializations
    const doctorsData = [
      // General Physicians
      {
        name: 'Dr. Vikramaditya Rathore',
        specialization: 'General Physician',
        hospital: smsHosp._id,
        consultationFee: 500,
    ...
    // Standard availability slots for doctors
    const standardSlots = [
    ...
    for (const doc of doctorsData) {
      ...
      // Add doctor reference to hospital
      await Hospital.findByIdAndUpdate(doc.hospital, {
        $push: { doctors: createdDoc._id }
      });
    ...
    const sampleAppointment = await Appointment.create({
      ...
      doctor: seededDoctors[0]._id, // Dr. Vikramaditya
      hospital: smsHosp._id,
    ...
    console.log(`✅ Seeded sample appointment (ID: ${sampleAppointment._id})`);
    console.log('🎉 Jaipur Healthcare Database Seeded Successfully!');
  } catch (error) {
